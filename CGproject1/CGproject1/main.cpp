...
static unsigned int CompileShader(unsigned int type, const std::string& source) {
	...
	if (result == GL_FALSE) {
		...
		std::cout << "Failed to compile" <<
			(type == GL_VERTEX_SHADER ? "verttex" : "fragment")
			<< " shader!" << std::endl;
...
void drag(int x, int y)
{
	...
	if (leftMouseButtonDown) {
		...
		cameraPos += xoffset * cameraSpeed * cameraFront;
		cameraPos += yoffset * glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
	}
		/*
		yaw += xoffset;
		pitch += yoffset;
